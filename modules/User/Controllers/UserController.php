...
class UserController extends FrontendController
{
    ...
    public function profileAvatarUpdate(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            'avatarImg' =>'required|image|mimes:jpeg,png,jpg,gif|max:2048',
        ]);
        if ($validator->passes()) {
            $avatarImg = $request->file('avatarImg');
            $avatarName = time() . '.' . $avatarImg->getClientOriginalExtension();
            $avatarImg->move(public_path('images/avatar'), $avatarName);
            // $avatarExtension = $avatarImg->getClientOriginalExtension();
            $user->avatar_id = $avatarName;
            $user->save();
